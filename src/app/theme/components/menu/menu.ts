    ...
    for (let i = 0; i < getPermissions.length; i++) {
        ...
        if (ScreenName == 'Dashboard') {
            ...
            if (DashboardReadPermission == true){
                conditionalhorizontalMenuItems.push(new Menu(1, 'Dashboard', '/mako/dashboard', null, 'dashboard', null, false, 0, "blue"))
            }
        }
    }
    for (let i = 0; i < getPermissions.length; i++) {
        ...
        if (ScreenName == 'QC') {
            ...
            if (QCReadPermission == true){
                conditionalhorizontalMenuItems.push(new Menu(2, 'QC', '/mako/qc', null, 'dashboard', null, false, 0, "blue"))
            }
        }
    }
    for (let i = 0; i < getPermissions.length; i++) {
    ...
    for (let i = 0; i < getPermissions.length; i++) {
        ...
        if (ScreenName == 'Admin') {
            ...
            if (AdminReadPermission == true){
                conditionalhorizontalMenuItems.push(new Menu(6, 'Admin', '/mako/admin', null, 'person', null, true, 0, "blue"),)
            }
        }
    }
    for (let i = 0; i < getPermissions.length; i++) {
